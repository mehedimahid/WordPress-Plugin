...
function ptmf_save_post($post_id){
	...
	$selected_term_id = $_POST["ptmf_terms"];
	if($selected_term_id>0){
		update_post_meta($post_id, "ptmf_selected_terms", $selected_term_id);
	}
	return $post_id;
}
...
function ptmf_display_metabox($post){
	...
	$selected_post_terms = get_post_meta($post->ID, "ptmf_selected_terms", true);
//	print_r( $selected_post_id);
//	print_r( $selected_post_terms);
	$args = array(
		"post_type" => "post",
		"posts_per_page"=>-1,
		"orderby" => "title",
...
//		"taxonomy" => "category",
	...
	foreach($_terms as $_term){
		$selected = ($_term->term_id==$selected_post_terms)? 'selected' : '';
		$terms_dropdown_list .= sprintf("<option %s value= '%s'>%s</option>",
			$selected,
			$_term->term_id,
			$_term->name
		);
	...
	$label2 = __("Select Category", "Post-Tax-Metafield");
	...
	$metabox_html = <<<HTML
				...
				<div class="field">
					<div class="field_c">
						<div class="label_c" >
							<label for="ptmf_terms">{$label2}:</label>
						</div>
						<div class="input_d"> 
						<select  name="ptmf_terms" id="ptmf_terms">
