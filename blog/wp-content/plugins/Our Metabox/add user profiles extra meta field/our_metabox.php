...
//		add_action("admin_enqueue_scripts",array($this, "omb_admin_assets"));
	...
	function omb_save_post_location($post_id){
		...
		$dp_color = isset($_POST["omb_dp_color"]) ?($_POST["omb_dp_color"]) : '';
		...
		update_post_meta($post_id, "omb_dp_color", $dp_color);
	...
	function omb_admin_menu(){
		...
		add_meta_box(
			"omb_image_info",
			__("Upload Image", "our-metabox"),
			array($this, "omb_image_info"),
			array("post")//where show this meta box
		);
		add_meta_box(
			"omb_gallery_info",
			__("Gallery Image", "our-metabox"),
			...
			$metabox_html = <<<HTML
				<div class="field">
					<div class="field_c">
						...
						<div class="input_c"> 
							<button type="button" id="upload_image">Upload Image</button>
	...
	function omb_gallery_info($post){
		...
		$image_url =esc_attr(get_post_meta($post->ID,"omb_images_url",true));
			...
			$button_label = __("Choose Images", "our-metabox");
			$metabox_html = <<<HTML
				<div class="field">
					<div class="field_c">
						<div class="label_c" >
						...
						<div class="input_c"> 
							<button type="button" id="upload_images">{$button_label}</button>
	...
	function omb_display_metabox($post){
		...
		$country = get_post_meta($post->ID, "omb_country1", true);
		...
		$dp_color = get_post_meta($post->ID, "omb_dp_color",true);
		...
		$metabox_html = <<<EOD
			...
			<p>
				<label for="omb_country1">{$label2}</label>
			
				<input type="text" id="omb_country1" name="omb_country1" value="{$country}" />
			</p>
			<p>
...
//EOD;
	// //or
		...
		foreach($colors as $color) {
			$checked = ($color == $dp_color) ? "selected='selected'" : '';
			$dropdown_html .= sprintf("<option value='%s' %s>%s</option>",$color,$checked,$color);
		}
		$metabox_html .= <<<EOD
<p>
        <label>{$label4}:</label>
        <select name="omb_dp_color" id="omb_dp_color">
        {$dropdown_html}
</select>
EOD;
