 ...
 * @package colm-demo
 */
//pre_get_posts হল একটি WordPress হুক যা থিম বা প্লাগইনের মাধ্যমে WordPress এর ডিফল্ট কোয়েরি পরিবর্তন বা কাস্টমাইজ করতে ব্যবহৃত হয়।
// এটি মূলত WordPress-এ ডেটা রিট্রিভ করার আগে কোয়েরি (query) পরিবর্তন করতে সাহায্য করে।
//
//restrict_manage_posts হল WordPress-এ একটি action hook যা অ্যাডমিন প্যানেলের পোস্ট তালিকা (post listing) পেজে কাস্টম ফিল্টার যোগ করতে ব্যবহার
// করা হয়। সাধারণত এটি পোস্ট, পেজ, বা কাস্টম পোস্ট টাইপ তালিকা প্রদর্শন করার সময় কাস্টম ড্রপডাউন, ইনপুট ফিল্ড বা অন্যান্য ফিল্টারিং অপশন যোগ করার জন্য ব্যবহার করা হয়।
...
//create thumbnail Filter Option
function colm_thumbnail_filter() {
	//only for post
	if ( isset( $_GET['post_type'] ) && $_GET['post_type'] != 'post' ) {
		return;
	}
	...
	$values       = array(
		"0" => __( "Thumbnail Status", "colm-demo" ),
		"1" => __( "Select Thumbnail", "colm-demo" ),
		"2" => __( "No Thumbnail", "colm-demo" )
	)
	?>
...
add_action( "restrict_manage_posts", "colm_thumbnail_filter" );

//Show filtered thumbnail
function colm_filter_thumbnail( $query ) {
	if ( ! is_admin() ) {
		return;
	}
	$filter_value = isset( $_GET['THFILTER'] ) ? $_GET['THFILTER'] : '';
...
add_action( "pre_get_posts", "colm_filter_thumbnail" );


//create Word Count Filter Option
function colm_wordcount_filter() {
	//only for post
	if ( isset( $_GET['post_type'] ) && $_GET['post_type'] != 'post' ) {
		return;
	}
	...
	$values       = array(
		"0" => __( "Word Count", "colm-demo" ),
		"1" => __( "Above 100 word", "colm-demo" ),
		"2" => __( "50 to 100", "colm-demo" ),
		"3" => __( "Below 50 ", "colm-demo" )
	)
	?>
...
add_action( "restrict_manage_posts", "colm_wordcount_filter" );


//Show filtered word count
function colm_filter_wordcount( $query ) {
	if ( ! is_admin() ) {
		return;
	}
	$filter_value = isset( $_GET['WCFILTER'] ) ? $_GET['WCFILTER'] : '';
...
add_action( "pre_get_posts", "colm_filter_wordcount" );
